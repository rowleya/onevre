 ...
 * param linkClass - the css class to use for the links
 * param selectFunc - the function called when an item is selected (takes 3
                      arguments - itemURL, helpID
 */
function Tree(name, lineHeight, selectColor, showIcon, expandAll, linkClass,
        selectFunc) {
    ...
    this.selectFromHelpURL = selectTreeNodeFromHelpURL;
}
 ...
 *
 */
function addTreeNode(parent, idnum, icon, content, helpID, URLData,
        expandType) {
...
function drawTree() {
    ...
    if (!browser.canDoDOM) {
        document.writeln("<ilayer id='tree_" + this.name
            + "' height=" + (this.totalNodes + 1)*this.lineHeight + ">");

        // create a selection layer
        document.writeln("<layer id='" + this.name
            + "Select' visibility='hide' bgColor='" + this.selectColor + "'>");
        document.writeln("</layer>");
    } else {
        if (browser.name == "Opera") {
            document.writeln("<div id=tree_" + this.name
                 + " style={visibility:visible;height:"
                 + (this.totalNodes + 1) * this.lineHeight + "}>");
        }
    }
...
function drawTreeNode(node) {
    ...
    if (browser.canDoDOM) {
        document.writeln("<div id=" + node.idnum
    ...
    for (var i=0 ; i < IsLineCount - 1 ; i++) {
        if (IsLine[i] == true ) {
            document.writeln("<td nowrap>"
                + "<img src='images/tree_linevertical.gif' border=0 width=16"
                + " height=22></td>");
        } else{
            document.writeln("<td nowrap><img src='images/tree_blank.gif'"
    ...
    if (IsLineCount > 0) {
        if (node.numChildren == 0) {
            ...
            if (IsLine[IsLineCount-1]) {
                ...
                document.writeln("<td nowrap>"
                    + "<img src='images/tree_linemiddlenode.gif' border=0"
                    + " width=16 height=22></td>");
            } else {
                document.writeln("<td nowrap>"
                    + "<img src='images/tree_linelastnode.gif' border=0"
                    + " width=16 height=22></td>");
            }
        } else {
            ...
            if (node.expand) {
                ...
                if (IsLine[IsLineCount-1]) {
                    ...
                    // not the end
                    document.writeln("<td nowrap><a href='#' onClick='return "
                        + this.name + ".toggle(\"" + node.idnum
                        + "\")'><img name='" + node.idnum
                        + "Toggle' src='images/tree_handledownmiddle.gif'"
                        + " border=0 width=16 height=22></a></td>");
                } else {
                    ...
                    document.writeln("<td nowrap><a href='#' onClick='return "
                        + this.name + ".toggle(\"" + node.idnum
                        + "\")'><img name='" + node.idnum
                        + "Toggle' src='images/tree_handledownlast.gif'"
                        + " border=0 width=16 height=22></a></td>");
                }
            } else {
                ...
                if (IsLine[IsLineCount-1]) {
                    ...
                    document.writeln("<td nowrap><a href='#' onClick='return "
                        + this.name + ".toggle(\"" + node.idnum
                        + "\")'><img name='" + node.idnum
                        + "Toggle' src='images/tree_handlerightmiddle.gif'"
                        + " border=0 width=16 height=22></a></td>");
                } else {
                    ...
                    document.writeln("<td nowrap><a href='#' onClick='return "
                        + this.name + ".toggle(\"" + node.idnum
                        + "\")'><img name='" + node.idnum
                        + "Toggle' src='images/tree_handlerightlast.gif'"
                        + " border=0 width=16 height=22></a></td>");
                }
            }
        }
    ...
    if (this.showIcon) {
        if (node.icon != "null") {
            document.writeln("<td nowrap><a href='#' onClick='return "
                + this.name + ".select(\"" + node.idnum + "\")'><img src='"
                + node.icon + "' border=0 width=19 height=22></td></a>");
        } else {
            if (node.numChildren == 0 ) {
                ...
                document.writeln("<td nowrap><a href='#' onClick='return "
                    ...
                    + "\")'><img src='images/tree_document.gif' border=0"
                    + " width=19 height=22></td></a>");
            } else {
                document.writeln("<td nowrap><a href='#' onClick='return "
                    + this.name+".select(\"" + node.idnum
                    + "\")'><img src='images/tree_folder.gif' border=0"
                    + " width=19 height=22></td></a>");
            }
        }
    }
    ...
    // create the content
    document.writeln("<td nowrap><a id='" + node.idnum
        + "Content' class='" + this.linkClass + "' href='#' onClick='return "
        + this.name + ".select(\"" + node.idnum + "\")'>" + node.content
        + "</a></td>");
    ...
    if (ancestorOpen) {
        ...
        if (browser.canDoDOM) {
            if (browser.name == "InternetExplorer"){
                ...
                if (browser.name == "Opera") {
                    document.getElementById(node.idnum).style.visibility =
                        'visible';
                    if (eval('document.getElementById("'
                            + node.idnum + '_NORM")') ) {
                        document.getElementById(
                            node.idnum + "_NORM").style.visibility = 'inherit';
                    }
                    document.getElementById(node.idnum).style.top =
                        (LineNumber) * this.lineHeight;

                } else {
                    document.getElementById(node.idnum).style.position =
                        'relative';
                    document.getElementById(node.idnum).style.visibility =
                        'visible';
                }
            ...
            eval('document.tree_' + this.name + '.layers["' + node.idnum
                + '"]').top = (LineNumber) * this.lineHeight;
            eval('document.tree_' + this.name + '.layers["' + node.idnum
                + '"]').visibility = 'show';
        }
    } else {
        ...
        if (browser.canDoDOM) {
            if (browser.name == "InternetExplorer") {
                ...
                if (browser.name == "Opera") {
                    document.getElementById(node.idnum).style.visibility =
                        'hidden';
                    document.getElementById(node.idnum).style.top =
                        this.totalNodes * this.lineHeight;
                } else {
                    document.getElementById(node.idnum).style.position =
                        'absolute';
                    document.getElementById(node.idnum).style.visibility =
            ...
            eval('document.tree_' + this.name + '.layers["' + node.idnum
                + '"]').top = this.totalNodes * this.lineHeight;
            eval('document.tree_' + this.name + '.layers["' + node.idnum
                + '"]').visibility = 'hide';
        }
    }
...
function toggleTreeNode(idnum) {
    ...
    if (browser.canDoDOM) {
        ...
        var holder = eval('document.tree_' + this.name + '.layers["'
            + node.idnum + '"]');
...
function scrollToTreeNode(node) {
    if (node.visible == true) {
        ...
        if (browser.canDoDOM) {
            if (browser.name == 'InternetExplorer') {
                nodeTop = document.body.scrollTop
                    + (this.lineHeight * node.lineNumber);
            } else {
                nodeTop = window.pageYOffset
                    + (this.lineHeight * node.lineNumber);
            }
        } else {
            var nodeArea = eval('document.tree_' + this.name + '.layers["'
                + node.idnum + '"]');
            nodeTop = nodeArea.pageY;
        }

        nodeBottom = nodeTop + (this.lineHeight * node.numChildren)
            + this.lineHeight;
...
function selectTreeNode(idnum) {
    ...
    if (browser.canDoDOM) {
        ...
        var selectLayer = eval('document.tree_' + this.name + '.layers["'
             + this.name + 'Select"]');
        var layer = eval ('document.tree_' + this.name + '.layers["'
             + idnum + '"]');
        var layerDoc = eval('document.tree_' + this.name + '.layers["'
             + idnum + '"].document');
    ...
    if (this.selectFunc) {
        if (node.URLData != "null") {
            this.selectFunc(node.URLData, node.helpID);
        }
    }
...
/**
 * Select a Tree Node from a HelpURL
 */
function selectTreeNodeFromHelpURL(helpURL) {
    var node = null;
    var index = helpURL.indexOf("http://");
    if (index != -1) {
        helpURL = helpURL.substring(7);
    }
    index = helpURL.indexOf('/');
    if (index != -1) {
        helpURL = helpURL.substring(index + 1);
    }
    if (this.selectedNode.URLData.indexOf(helpURL) == -1) {
        for (var i = 0; i < this.topNodesCount; i++) {
            node = this.topNodes[i].findHelpURL(helpURL);
            if (node != null) {
                break;
            }
        }
...
function TreeNode(idnum, icon, content, helpID, URLData, expandType) {
    ...
    this.expand = true;
    if (expandType == "0" || expandType == false) {
        this.expand == false;
    }
    this.visible = false;
...
/**
 * Recursively find a url that matches helpURL
 *
 * param helpURL - HelpURL to find
 *
 * returns the matching Node or null if no matches
 */
function findHelpURL(helpURL) {

    // if this node matches return it
    if (this.URLData.indexOf(helpURL) != -1) {
        return this;
    }

    // humm no matches see if the children match
    for (var i = 0; i < this.numChildren; i++) {
        var node = this.children[i].findHelpURL(helpURL);
        if (node != null) {
            return node;
        }
    }
