...
public class MulticastAddressAllocator {
    ...
    public static final String SDR_BASE_ADDRESS = "224.2.128.0";
    public static final int SDR_MASK_SIZE = 17;

    private static Vector<String> allocatedAddresses = new Vector<String>();

    private static int makeIntFromByte4(byte[] b) {
        return b[0] << 24 | (b[1] & 0xff) << 16 | (b[2] & 0xff) << 8 | (b[3] & 0xff);
    }

    private static byte[] makeByte4FromInt(int i) {
    ...
    public static String allocateAddress() {
        try {
            ...
            do {
                addressString = Inet4Address.getByAddress(
                        randomAddress(SDR_BASE_ADDRESS, SDR_MASK_SIZE)).getHostAddress();
    ...
    private MulticastAddressAllocator() {
